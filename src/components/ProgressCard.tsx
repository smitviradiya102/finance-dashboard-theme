...
interface ProgressCards {
  ...
  export default function ProgressCard({ icon, title, saved, percent, target }: ProgressCards) {
    return (
      <div className="w-[251px] h-[103px] gap-[16px] rounded-[12px] border border-[#E5E6E6] p-[14px] flex items-end flex-col">
        ...
        <div className="w-[223px] h-[31px] gap-[10px]">
          ...
          <div className="flex justify-between items-end mt-1">
            <p className="font-semibold text-[10px] text-[#242E2C] pt-1">{saved} <span className="text-[#6B7271]">{percent}</span></p>
            <p className="font-normal text-[10px] text-[#6B7271]">Target: <span className="text-[#242E2C]">{target}</span></p>
          </div>
        </div>
      </div>
    );
